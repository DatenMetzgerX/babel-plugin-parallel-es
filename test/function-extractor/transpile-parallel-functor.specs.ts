...
describe("TranspileParallelFunctor", function() {
      ...
      const transpileCall = () =>
        transpile(
          `
                ...
                `,
          "program.body[2].expression",
          "program.body[1].declarations[0].init.properties[1].value",
          { functorPathRelative: false }
        );
  ...
  function transpile(
    ...
    callPath: string,
    functorPath: string,
    userOptions?: { functorPathRelative?: boolean; methodName?: string }
  ): {
    ast: t.Node;
    ...
    function transpileIfFunctorAndCallCollected(current: NodePath<any>) {
      if (result.callExpression && result.originalFunctor) {
        const methodName =
          options.methodName ||
          ((result.callExpression.node.callee as t.MemberExpression).property as t.Identifier).name;
        const transpileResult = transpileParallelFunctor(
          result.originalFunctor,
          registry,
          isParallelFunctor(PARALLEL_METHODS[methodName])
        );
