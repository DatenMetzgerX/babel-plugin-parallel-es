...
describe("ImportsRegistry", function() {
  ...
  describe("addImport", function() {
    it("adds the named import", function() {
      ...
      expect(imports)
        .to.have.property("lodash")
        .that.eql([{ imported: "map", local: "lodashMap", references: [reference] }]);
    });
    ...
    it("merges equal imports", function() {
      ...
      expect(imports)
        .to.have.property("lodash")
        .that.eql([{ imported: "map", local: "lodashMap", references: [ref1, ref2] }]);
    });
    ...
    it("distinguishes imports of different modules", function() {
      ...
      expect(imports)
        .to.have.property("lodash")
        .that.eql([{ imported: "map", local: "lodashMap", references: [lodashMap] }]);
      expect(imports)
        .to.have.property("underscore")
        .that.eql([{ imported: "map", local: "underscoreMap", references: [underscoreMap] }]);
    });
  });
  ...
  describe("addDefaultImport", function() {
    it("adds an import for default", function() {
      ...
      expect(imports)
        .to.have.property("lodash")
        .that.eql([{ imported: "default", local: "_", references: [ref] }]);
    });
  });
  ...
  describe("addNamespaceImport", function() {
    it("adds an import for the defined namespace", function() {
      ...
      expect(imports)
        .to.have.property("lodash")
        .that.eql([{ imported: "*", local: "_", references: [ref] }]);
    });
  });
});
