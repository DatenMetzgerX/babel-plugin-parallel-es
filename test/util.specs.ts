...
describe("util", function() {
  describe("createFunctionId", function() {
    it("returns a serialized function id", function() {
      ...
      const functionId = evaluate(functionIdAst);
  ...
  describe("createSerializedFunctionCall", function() {
    it("returns a serialized function call", function() {
      ...
      const functionCall = evaluate(serializedCallNode);
  ...
  describe("toFunctionDeclaration", function() {
    ...
    it("returns the unmodified function declaration", function() {
      ...
      const functionDeclaration = t.functionDeclaration(
        t.identifier("test"),
        [t.identifier("x") as any],
        t.blockStatement([t.returnStatement(t.identifier("x)"))])
      );
    ...
    it("fails for other function types", function() {
      ...
      const method = t.objectMethod(
        ...
        t.identifier("test"),
        [],
        t.blockStatement([t.returnStatement(t.stringLiteral("test"))])
      );

      // act, assert
      expect(() => toFunctionDeclaration(method, scope)).to.throw(
        "Not supported conversion of ObjectMethod to a FunctionDeclaration."
      );
    });
  });
