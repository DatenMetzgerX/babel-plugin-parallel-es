...
function addImports(module: Readonly<ModuleFunctionsRegistry>, path: NodePath<t.Node>) {
...
function registerEntryFunction(definition: IEntryFunctionRegistration): t.Statement {
  ...
  const registerStaticFunctionMember = t.memberExpression(
    t.identifier("slaveFunctionLookupTable"),
    t.identifier("registerStaticFunction")
  );
  ...
  return {
    Statement(path: NodePath<t.Statement>) {
      ...
      for (const module of registry.modules) {
        ...
        const body: t.Statement[] = module.functions.slice();
        ...
        const wrapper = t.expressionStatement(
          t.callExpression(t.functionExpression(undefined, [], t.blockStatement(body)), [])
        );
